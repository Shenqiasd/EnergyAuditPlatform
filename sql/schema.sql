...
CREATE TABLE `tpl_tag_mapping` (
    ...
    `mapping_type`        VARCHAR(32)  DEFAULT 'SCALAR'        COMMENT '映射类型(SCALAR/TABLE/EQUIPMENT_BENCHMARK)',
    `source_type`         VARCHAR(16)  DEFAULT 'CELL_TAG'      COMMENT '来源类型(CELL_TAG/TABLE_RANGE)',
    `row_key_column`      INT          DEFAULT NULL            COMMENT '行键列索引(TABLE类型)',
    `column_mappings`     TEXT         DEFAULT NULL            COMMENT '列映射JSON(TABLE类型)',
    `header_row`          INT          DEFAULT NULL            COMMENT '表头行号(TABLE类型)',
    `remark`              VARCHAR(256) DEFAULT NULL            COMMENT '备注',
    `deletion_source`     VARCHAR(16)  DEFAULT NULL            COMMENT '删除来源(USER/SYNC)',
    `create_by`           VARCHAR(64)  DEFAULT NULL            COMMENT '创建人',
    `create_time`         DATETIME     DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_by`           VARCHAR(64)  DEFAULT NULL            COMMENT '更新人',
    `update_time`         DATETIME     DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `tpl_submission` (
    ...
    `status`           TINYINT      DEFAULT 0               COMMENT '状态(0草稿 1已提交 2已通过 3已退回)',
    `review_comment`   VARCHAR(512) DEFAULT NULL            COMMENT '审核意见(退回时填写)',
...
CREATE TABLE `de_tech_indicator` (
    ...
    `unit_output_energy`        DECIMAL(18,6) DEFAULT NULL            COMMENT '单位产值综合能耗(当量值)',
    `unit_output_energy_equal`  DECIMAL(18,6) DEFAULT NULL            COMMENT '单位产值综合能耗(等价值)',
    ...
    `coal_target`               DECIMAL(18,4) DEFAULT NULL            COMMENT '本期考核指标(等价值)',
    `coal_actual`               DECIMAL(18,4) DEFAULT NULL            COMMENT '本期考核指标(强度等价值)',
    `employee_count`            INT           DEFAULT NULL            COMMENT '从业人员(人)',
    `energy_manager_count`      INT           DEFAULT NULL            COMMENT '能源管理师人数(人)',
    `total_energy_equiv_excl_green` DECIMAL(18,4) DEFAULT NULL        COMMENT '综合能耗扣除绿电(当量值)(吨标煤)',
    `total_energy_equal_excl_green` DECIMAL(18,4) DEFAULT NULL        COMMENT '综合能耗扣除绿电(等价值)(吨标煤)',
    `raw_material_energy`       DECIMAL(18,4) DEFAULT NULL            COMMENT '原材料用能(吨标煤)',
    `electrification_rate`      DECIMAL(8,4)  DEFAULT NULL            COMMENT '电气化率(%)',
    `total_energy_equal_excl_material` DECIMAL(18,4) DEFAULT NULL     COMMENT '综合能耗扣除原料(等价值)(吨标煤)',
    `create_by`                 VARCHAR(64)   DEFAULT NULL            COMMENT '创建人',
    `create_time`               DATETIME      DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_by`                 VARCHAR(64)   DEFAULT NULL            COMMENT '更新人',
    `update_time`               DATETIME      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `de_product_unit_consumption` (
    ...
    `indicator_name`          VARCHAR(128)  DEFAULT NULL            COMMENT '指标名称(如:XX单位产量综合能耗)',
    `indicator_unit`          VARCHAR(64)   DEFAULT NULL            COMMENT '指标单位(如:千克标准煤/部)',
    `numerator_unit`          VARCHAR(32)   DEFAULT NULL            COMMENT '分子项单位(千克标准煤)',
    `denominator_unit`        VARCHAR(32)   DEFAULT NULL            COMMENT '分母项单位(如:部)',
    `year_type`               VARCHAR(16)   DEFAULT NULL            COMMENT '年份(已弃用，本年+上年存同一行)',
    `measurement_unit`        VARCHAR(32)   DEFAULT NULL            COMMENT '计量单位',
    `output`                  DECIMAL(18,4) DEFAULT NULL            COMMENT '产量(本年度分母项值)',
    `energy_consumption`      DECIMAL(18,4) DEFAULT NULL            COMMENT '能源消耗量(本年度分子项值,吨标煤)',
    `unit_consumption`        DECIMAL(18,6) DEFAULT NULL            COMMENT '单耗(本年度指标值)',
    `prev_unit_consumption`   DECIMAL(18,6) DEFAULT NULL            COMMENT '上年度单耗(指标值)',
    `prev_energy_consumption` DECIMAL(18,4) DEFAULT NULL            COMMENT '上年度能源消耗量(分子项值)',
    `prev_output`             DECIMAL(18,4) DEFAULT NULL            COMMENT '上年度产量(分母项值)',
    `create_by`               VARCHAR(64)   DEFAULT NULL            COMMENT '创建人',
    `create_time`             DATETIME      DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_by`               VARCHAR(64)   DEFAULT NULL            COMMENT '更新人',
    `update_time`             DATETIME      DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
