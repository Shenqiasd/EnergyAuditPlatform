...
export const SCHEMA_REGISTRY: Record<string, TableSchema> = {
  ...
  de_tech_reform_history: {
    ...
    fields: {
      ...
      payback_period:      { label: '投资回收期(年)', type: 'DECIMAL' },
      completion_date:     { label: '完成时间', type: 'DATE' },
      actual_saving:       { label: '实际节能量(吨标煤)', type: 'DECIMAL' },
  ...
  de_equipment_energy: {
    ...
    fields: {
      ...
      model_spec:                     { label: '型号规格', type: 'STRING' },
      nameplate_output:               { label: '铭牌出力', type: 'STRING' },
      main_energy_name:               { label: '主要能源名称', type: 'STRING' },
      main_energy_consumption:        { label: '主要能源消费量', type: 'DECIMAL' },
      avg_operating_efficiency:       { label: '平均运行效率', type: 'DECIMAL' },
      residual_heat_energy:           { label: '余热余能量', type: 'DECIMAL' },
      available_residual_heat_energy: { label: '可回收余热余能量', type: 'DECIMAL' },
      utilized_residual_heat_energy:  { label: '已利用余热余能量', type: 'DECIMAL' },
      recovery_utilization_rate:      { label: '回收利用率', type: 'DECIMAL' },
      statistical_load_rate:          { label: '统计负荷率', type: 'DECIMAL' },
      test_efficiency:                { label: '测试效率', type: 'DECIMAL' },
      flue_gas_loss_rate:             { label: '排烟热损失率', type: 'DECIMAL' },
      heat_loss_rate:                 { label: '散热损失率', type: 'DECIMAL' },
      other_loss:                     { label: '其他损失', type: 'DECIMAL' },
  ...
  de_product_unit_consumption: {
    ...
    fields: {
      ...
      indicator_unit:          { label: '指标单位', type: 'STRING' },
      numerator_unit:          { label: '分子项单位', type: 'STRING' },
      denominator_unit:        { label: '分母项单位', type: 'STRING' },
      measurement_unit:        { label: '计量单位', type: 'STRING' },
      output:                  { label: '产量(本年度)', type: 'DECIMAL' },
      energy_consumption:      { label: '能源消耗量(本年度,吨标煤)', type: 'DECIMAL' },
      unit_consumption:        { label: '单耗(本年度)', type: 'DECIMAL' },
      prev_unit_consumption:   { label: '单耗(上年度)', type: 'DECIMAL' },
      prev_energy_consumption: { label: '能源消耗量(上年度)', type: 'DECIMAL' },
      prev_output:             { label: '产量(上年度)', type: 'DECIMAL' },
    },
  },
  ...
  de_saving_calculation_detail: {
    label: '节能量计算产品级明细',
    fields: {
      product_seq:      { label: '产品序号', type: 'NUMBER' },
      row_type:         { label: '行类型', type: 'STRING' },
      row_label:        { label: '项目', type: 'STRING' },
      product_name:     { label: '产品名称', type: 'STRING' },
      measurement_unit: { label: '单位', type: 'STRING' },
      current_value:    { label: '审计期值', type: 'DECIMAL' },
      base_value:       { label: '基准期值', type: 'DECIMAL' },
    },
  },
  ...
  de_management_policy: {
    ...
    fields: {
      ...
      valid_period: { label: '有效期', type: 'DATE' },
      main_content: { label: '主要内容', type: 'TEXT' },
      remark:       { label: '备注', type: 'STRING' },
    },
  },
