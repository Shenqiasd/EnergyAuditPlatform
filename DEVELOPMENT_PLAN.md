...
| Wave 12 | 腾讯云部署与商业许可证接入 | 2 | 2 周 |
| **合计** | | **30 个 Sprint / PR** | **~30 周** |
...
## Wave 12 — 腾讯云部署与商业许可证接入
> 目标：形成面向腾讯云生产环境的可部署基线，并完成 SpreadJS 商业 License 的安全接入方案，确保本地开发配置不污染生产部署。

### Sprint 12.1 — 腾讯云部署基线
**PR 名称：** `feat: deployment - tencent cloud production baseline`

**架构与运维：**
- [ ] 明确腾讯云部署拓扑：前端静态站点 / Nginx / Spring Boot / MySQL / OnlyOffice / 对象存储（如 COS）边界
- [ ] 产出生产环境配置分层方案：`dev / test / prod` 环境变量、配置文件、密钥管理边界
- [ ] 设计域名、HTTPS 证书、反向代理与跨域策略
- [ ] 设计进程管理与发布方式：Docker Compose 或容器化方案、健康检查、日志采集、异常重启
- [ ] 设计数据库备份、文件存储备份、回滚策略
- [ ] 补充腾讯云部署文档：服务器初始化、依赖安装、发布步骤、验收步骤

**研发约束：**
- [ ] 明确本地开发专用配置与生产配置隔离规则，禁止将本地调试地址、测试密钥、临时脚本写入生产默认配置
- [ ] 明确部署前检查清单：端口、环境变量、第三方服务可达性、文件目录权限、日志目录

**DoD：** 可基于文档在腾讯云新环境完成一次标准化部署，核心服务健康检查与基础访问链路验证通过。

---

### Sprint 12.2 — 商业 License 接入
**PR 名称：** `feat: deployment - commercial license integration for production`

**SpreadJS / 商业组件：**
- [ ] 设计 SpreadJS 生产资产交付方案：优先本地静态资源或私有制品存储，避免生产环境依赖公网 CDN
- [ ] 设计 SpreadJS 商业 License 注入方案：通过安全配置或密钥管理注入，不写入 Git 仓库
- [ ] 梳理前端模板设计器、企业填报页、构建流程对商业 License 的加载时机
- [ ] 明确开发环境 / 测试环境 / 生产环境的 License 差异策略，避免本地临时方案影响腾讯云正式部署
- [ ] 增加 License 失效或资源加载失败时的降级提示与排障说明

**发布验证：**
- [ ] 形成生产构建验证清单：模板设计器可打开、填报页可加载、无 CDN 依赖、控制台无 License 相关报错
- [ ] 形成回滚方案：License 配置回退、静态资源回退、前端版本回退
- [ ] 补充商业组件合规说明：License 文件保管、人员权限、续期与替换流程

**DoD：** 在腾讯云部署环境中，SpreadJS 设计器与填报功能可在商业 License 下稳定运行，且发布包不依赖公网 CDN。
